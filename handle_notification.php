...
function updateStatusAndNotify($conn, $type, $id, $user_id, $status, $reservation_id = null, $reject_reason = null) {
    ...
    if ($type === 'reservation' && $status === 'rejected') {
        $sql_update = "UPDATE reservation SET status = ?, message = ? WHERE reservation_id = ?";
        $stmt_update = mysqli_prepare($conn, $sql_update);
        mysqli_stmt_bind_param($stmt_update, "ssi", $status, $reject_reason, $id);
    } else {
